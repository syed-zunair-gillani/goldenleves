...
const RadioButton = ({ children, value, onClick = () => {}, ...rest }: any) => {
  ...
  return (
    <div
      className={`inline-block flex-1 h-fit rounded-lg p-[1px]
    ...
    >
      <div className={`bg-[#111111] shadow-md overflow-hidden rounded-lg`}>
        ...
        >
          <div
            className={`border-[1px] flex justify-center items-center w-5 h-5 rounded-full 
          ${isActive ? "border-[#f8d7a7]" : "border-[#A1A1A1]"}
        `}
          >
            {isActive && (
              <div className="golden-gradient-bg w-[13px] h-[13px] rounded-full" />
            )}
          </div>
          {children}
        </button>
